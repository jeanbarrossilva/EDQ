...
class QuoteNotificationBroadcastReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        QuoteModel.unreadQuotes().random().let { quote ->
            val favoriteQuoteActionIntent = Intent(context, QuoteNotificationFavoriteQuoteActionBroadcastReceiver::class.java).apply {
                putExtra(INTENT_EXTRA_KEY_FAVORITE_QUOTE, quote)
            }
            ...
            Notification.Builder(context, MainViewModel.NOTIFICATION_CHANNEL_ID_QUOTES)
                ...
                .build()
                .let { notification ->
                    context?.notificationManager?.notify(quote.numberId, notification)
                    quote.isRead = true
                }
        }
    }
